...
export function normalizeBadWords(words: unknown[] = []): string[] {
  return [
    ...new Set(
...
export function normalizeAutomodConfig(raw: Partial<AutomodConfig> = {}): AutomodConfig {
...
export function uppercaseRatio(text = ''): number {
...
export function matchBadWord(text: string, words: string[]): string {
...
export function evaluateAutomodMessage({
  content,
  mentionCount = 0,
  config = defaultAutomodConfig()
}: AutomodMessageInput): AutomodMessageMatch | null {
...
export function evaluateSpamActivity(
  timestamps: number[],
  now: number,
  config: Partial<AutomodConfig> = defaultAutomodConfig()
): AutomodSpamEvaluation {
  const settings = normalizeAutomodConfig(config);
