...
export function createConfig(env: EnvLike = process.env): AppConfig {
  ...
  return {
    ...
    aiTimeoutMs: parseNumber(env.AI_TIMEOUT_MS, 60000, { min: 1000 }),
    aiMentionEnabled: parseBoolean(env.AI_MENTION_ENABLED || 'true'),
    aiMentionCooldownSeconds: parseNumber(env.AI_MENTION_COOLDOWN_SECONDS, 30, { min: 3 }),
    aiMentionMaxChars: parseNumber(env.AI_MENTION_MAX_CHARS, 700, { min: 50 }),
    openAiApiKey: '',
    openRouterApiKey,
...
export function summarizeConfig(config: AppConfig): string[] {
  ...
  return [
    ...
    `- AI mentions: ${config.aiEnabled && config.aiMentionEnabled ? `enabled (${config.aiMentionCooldownSeconds}s cooldown)` : 'disabled'}`,
